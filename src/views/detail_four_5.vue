  ...
  <div class="detail">
    <el-row :gutter="30">
      ...
      <el-col :span="8" class="back1"
        ><img src="../assets/images/zhuangpeiji.png" width="100%" alt=""
      /></el-col>
      <el-col :span="8"
        ><div class="back2" style="margin-bottom: 20px">
          <template>
...
const { zhuangpeixian_368_only } = require("../../api/index.js");
export default {
  name: "detail_four_1",
  data() {
    return {
      ...
      filterList_4: ["item_alarm", "totaltime_alarm", "item_rejects"]
    };
  },
  async mounted() {
    const { data } = await zhuangpeixian_368_only({
      num: this.$route.params.id
    });

    data.data.forEach(val => {
      const num = val.tagname.split("\\").length;
      val.tagname = val.tagname.split("\\")[num - 1];
      this.nameFilter(val);
    });
